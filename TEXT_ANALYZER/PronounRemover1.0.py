...
def ReformSentence(structured_sentence):
    print "In Reform Sentence"
    sentence = ""
    for Chunk in structured_sentence:
        if type(Chunk) == tuple:
            print "Got a Chunk of Type tuple, Adding part to sentence"
            print Chunk
            print Chunk[0]
            sentence += Chunk[0] + " "
        else:
            print "Got a non small Chunk, Going in deeper, checking inside..."
            print Chunk
            sentence += ReformSentence(Chunk)
    print "Done Getting Words, here's what we got"
    print sentence
    return sentence
...
def Process(string):
	...
	try:
		    ...
		    structured_sentence = cp.parse(tagged)
                    ...
                    print mentioned_people.People
                    pronoun_removed_sentence += ReformSentence(structured_sentence)
                print pronoun_removed_sentence
                return pronoun_removed_sentence
